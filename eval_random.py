...
for method_name in methods:
    #クラスタ数とトータルコア数を固定
    with open(f'random_new_4_{method_name}.txt', 'w') as f:
            ...
            for cluster_total_core in cluster_total_cores:
                ...
                # for m in range(40, 41, 20):
                    for n in range(dag_num):
                        ...
                        ave_response_time = simulator.Scheduling(task_name='random')
                ...
                f.write(f"random_result_response_times_{cluster_num}_{cluster_core_num}={response_times_for_hako}\n")
                f.write(f"random_result_response_times_c_{cluster_num}_{cluster_core_num}={response_times_for_hako_c}\n")
